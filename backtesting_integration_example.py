...
from src.backtesting.config import BacktestConfig, get_scenario_config, create_custom_config
...
def run_scenario_comparison_example():
    ...
    for scenario_name in scenarios:
        ...
        try:
            ...
            print(f"  Total Trades: {metrics['total_trades']}")
            print(f"  Win Rate: {metrics['win_rate']:.2%}")
            print(f"  Total Return: ${metrics['total_return']:.2f} ({metrics['total_return_percentage']:.2f}%)")
    ...
    print("="*60)
    print(f"{'Scenario':<12} {'Trades':<8} {'Win Rate':<10} {'Return %':<10} {'Sharpe':<8}")
    print("-" * 60)
    
    for scenario_name in scenarios:
        if scenario_results[scenario_name]:
            metrics = scenario_results[scenario_name]['metrics']
            print(f"{scenario_name:<12} {metrics['total_trades']:<8} "
...
def run_simple_backtest_example():
    ...
    try:
        ...
        if metrics['total_trades'] > 0:
            print(f"Average Trade Duration: {metrics['average_trade_duration_hours']:.1f} hours")
            print(f"Profit Factor: {metrics['profit_factor']:.2f}")
