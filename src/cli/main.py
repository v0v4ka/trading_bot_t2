...
def build_parser() -> argparse.ArgumentParser:
    ...
    backtest_parser.add_argument(
        "--days", type=int, default=60, help="Number of days to backtest"
    )
    ...
    visualize_parser.add_argument(
        "--days", type=int, default=60, help="Number of days of data"
    )
    visualize_parser.add_argument(
        "--theme",
        choices=[t.value for t in ChartTheme],
        default="light",
        help="Chart theme",
    )
    ...
    visualize_parser.add_argument(
        "--backtest", action="store_true", help="Include backtesting results"
    )
...
def main(argv: List[str] | None = None) -> None:
    ...
    if args.command == "backtest":
        ...
        if results.trades:
            ...
            if results.equity_curve:
                ...
                total_return = (
                    (final_value - args.initial_capital) / args.initial_capital
                ) * 100
    ...
    if args.command == "visualize":
        ...
        if args.backtest:
            ...
            print(
                f"  Generated {len(backtest_results.trades)} trades for visualization"
            )
        ...
        config = ChartConfig(figure_size=(args.width, args.height))
        ...
        if args.backtest and backtest_results:
            ...
            fig = visualizer.create_integrated_chart(
                data=df, title=title, save_path=output_path
            )
