...
class TradingBotConfig:
    ...
    def __init__(self, env_file: Optional[str] = None):
        ...
        self.default_initial_capital = get_env_var("DEFAULT_INITIAL_CAPITAL", 10000.0, float)
        self.default_backtest_days = get_env_var("DEFAULT_BACKTEST_DAYS", 60, int)
        self.default_symbol = get_env_var("DEFAULT_SYMBOL", "AAPL")
        
        # Agent Configuration
        self.signal_confidence_threshold = get_env_var("SIGNAL_CONFIDENCE_THRESHOLD", 0.6, float)
        self.decision_confidence_threshold = get_env_var("DECISION_CONFIDENCE_THRESHOLD", 0.7, float)
        ...
        # Directory Paths
        self.config_dir = get_env_var("CONFIG_DIR", "config/")
    ...
    def validate(self) -> list[str]:
        ...
        if self.decision_confidence_threshold < 0 or self.decision_confidence_threshold > 1:
