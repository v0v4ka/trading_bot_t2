...
class PerformanceAnalyzer:
    ...
    def _calculate_trade_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "win_rate": (
                len(winning_trades) / self.results.total_trades
    ...
    def _calculate_return_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_trade_return": (
                np.mean(trade_returns) * 100 if trade_returns else 0.0
            ),
            "median_trade_return": (
                np.median(trade_returns) * 100 if trade_returns else 0.0
            ),
            "best_trade": (
                max(self.results.trades, key=lambda t: t.pnl).pnl
                if self.results.trades
                else 0.0
            ),
            "worst_trade": (
                min(self.results.trades, key=lambda t: t.pnl).pnl
                if self.results.trades
                else 0.0
            ),
        }
    ...
    def _calculate_additional_stats(self) -> Dict[str, Any]:
        ...
        stop_loss_trades = [
            trade for trade in self.results.trades if trade.stop_loss_triggered
        ]
    ...
    def generate_report(self, detailed: bool = True) -> str:
        ...
        report.append(
            f"Max Drawdown: ${metrics['max_drawdown']:.2f} ({metrics.get('max_drawdown_percentage', 0):.2f}%)"
        )
        ...
        if detailed and metrics["total_trades"] > 0:
            ...
            report.append(
                f"Average Trade Return: {metrics['average_trade_return']:.2f}%"
            )
            ...
            if "stop_loss_trades" in metrics:
                report.append(
                    f"Stop Loss Triggered: {metrics['stop_loss_trades']} ({metrics['stop_loss_rate']:.2%})"
                )
