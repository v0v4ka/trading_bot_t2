...
from .metrics import BacktestResults, Trade, PerformanceAnalyzer
...
class BacktestEngine:
    ...
    def __init__(self, config: BacktestConfig, decision_logger: Optional[DecisionLogger] = None):
        """
        ...
        logger.info(f"Initialized BacktestEngine for {config.symbol} from {config.start_date} to {config.end_date}")
    ...
    def run_backtest(self) -> BacktestResults:
        ...
        try:
            ...
            logger.info(f"Backtesting completed. Total trades: {len(self.results.trades)}")
            return self.results
    ...
    def _load_historical_data(self) -> pd.DataFrame:
        ...
        df = self.data_provider.get_historical_data(
            symbol=self.config.symbol,
            period=None,  # Use start/end dates instead
            interval=self.config.timeframe,
        ...
        if df.empty:
            raise ValueError(f"No data available for {self.config.symbol} in specified date range")
    ...
    def _initialize_agents(self, df: pd.DataFrame) -> None:
        ...
        self.signal_agent = SignalDetectionAgent(
            df=df,
            llm_client=None if not self.config.use_llm_confirmation else None  # Use default LLM
        )
    ...
    def _get_signals_at_timestamp(self, index: int, df: pd.DataFrame) -> List[Signal]:
        ...
        try:
            ...
            # Get signals from Signal Detection Agent
            signals = self.signal_agent.detect_signals(
                confidence_threshold=self.config.signal_confidence_threshold
            )
            
            # Filter signals that occur at current timestamp
            current_timestamp = df.iloc[index]['timestamp'] if 'timestamp' in df.columns else df.iloc[index].name
    ...
    def _get_decisions(self, signals: List[Signal], current_bar: pd.Series) -> List[TradingDecision]:
        ...
        try:
            ...
            # Get decisions from Decision Maker Agent
            decisions = self.decision_agent.make_decisions(signals, ohlcv_data)
    ...
    def _execute_trades(self, decisions: List[TradingDecision], current_bar: pd.Series, timestamp: datetime) -> None:
        """Execute trades based on decisions."""
        for decision in decisions:
            if decision.action in ["BUY", "SELL"]:
                self._execute_single_trade(decision, current_bar, timestamp)
    
    def _execute_single_trade(self, decision: TradingDecision, current_bar: pd.Series, timestamp: datetime) -> None:
        ...
        try:
            ...
            position = {
                ...
                "stop_loss_price": self._calculate_stop_loss(current_price, decision.action),
            }
            ...
            logger.info(f"Executed {decision.action} trade at {timestamp}: {quantity:.4f} shares at ${current_price:.2f}")
            
        except Exception as e:
            logger.error(f"Failed to execute trade: {str(e)}")
    
    def _close_position_immediately(self, position: Dict[str, Any], current_bar: pd.Series, timestamp: datetime) -> None:
        ...
        exit_price = entry_price * (1 + np.random.normal(0, 0.02))  # ±2% random variation
        ...
        if position["trade_type"] == "BUY":
            ...
            proceeds = position["entry_price"] * position["quantity"] + (position["entry_price"] - exit_price) * position["quantity"]
        
        self.current_capital += proceeds - position["transaction_cost"]  # Second transaction cost
    ...
    def _close_position_stop_loss(self, position: Dict[str, Any], current_bar: pd.Series) -> None:
        ...
        self.results.trades.append(trade)
        logger.info(f"Stop loss triggered for {position['trade_type']} position at ${stop_loss_price:.2f}")
...
def run_simple_backtest(symbol: str = "AAPL", days: int = 365, **config_overrides) -> BacktestResults:
    """
