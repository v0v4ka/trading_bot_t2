...
class ChartVisualizer:
    ...
    def __init__(self, config: Optional[ChartConfig] = None, theme: Optional[ChartTheme] = None):
        """
    ...
    def _plot_candlesticks(self, ax: plt.Axes, df: pd.DataFrame) -> None:
        ...
        # Plot candle bodies
        width = mdates.date2num(df.index[1]) - mdates.date2num(df.index[0]) if len(df) > 1 else 1
        ...
        for timestamp, row in df.iterrows():
            ...
            color = (self.config.candle_up_color if row['Close'] >= row['Open'] 
                ...
                if timestamp in df.index:
                    ...
                    if fractal['type'] == 'up':
                        ax.scatter(
                            ...
                            label='Fractal Up' if 'Fractal Up' not in existing_labels else "",
                            zorder=5
                        )
                    else:  # down fractal
                        ax.scatter(
                            ...
                            label='Fractal Down' if 'Fractal Down' not in existing_labels else "",
                            zorder=5
                        )
    ...
    def _plot_awesome_oscillator(self, ax: plt.Axes, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            colors = [
                self.config.awesome_oscillator_positive_color if val >= 0 
    ...
    def _plot_agent_decisions(self, ax: plt.Axes, df: pd.DataFrame, backtest_results: BacktestResults) -> None:
    ...
    def _plot_trade_annotations(self, ax: plt.Axes, df: pd.DataFrame, backtest_results: BacktestResults) -> None:
        ...
        for i, trade in enumerate(trades):
            if trade.entry_time in df.index and trade.exit_time in df.index:
                ...
                line_color = (self.config.profit_line_color if pnl > 0 
    ...
    def _plot_decision_markers_ao(self, ax: plt.Axes, df: pd.DataFrame, backtest_results: BacktestResults) -> None:
        ...
        try:
            ...
            for trade in trades:
                if trade.entry_time in df.index:
                    ao_value = ao_data.loc[trade.entry_time] if trade.entry_time in ao_data.index else 0
                    
                    color = (self.config.buy_decision_color if trade.trade_type == 'BUY' 
    ...
    def _plot_equity_curve(self, ax: plt.Axes, backtest_results: BacktestResults) -> None:
    ...
    def _configure_chart_appearance(self, fig: plt.Figure, df: pd.DataFrame, title: Optional[str]) -> None:
    ...
    def _add_performance_metrics_box(self, fig: plt.Figure, backtest_results: BacktestResults) -> None:
        ...
        winning_trades = sum(1 for trade in backtest_results.trades 
