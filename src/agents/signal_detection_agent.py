...
class SignalDetectionAgent:
    ...
    def __init__(self, df: pd.DataFrame, llm_client: Optional[Callable[[str], float]] = None):
        self.df = df
    ...
    def _build_prompt(self, fractal: Dict, ao_value: float) -> str:
        template_path = os.path.join(os.path.dirname(__file__), '..', '..', 'prompts', 'signal_confirmation_prompt.txt')
